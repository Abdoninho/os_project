...
int Directory::getMySizeOnDisk() {
    ...
    Directory_Entry entry(dir_name, dir_attr, dir_firstCluster);// عدد المدخلات (ملفات وفولدرات)
    int totalEntries = DirOrFiles.size();
    int sizeInBytes = totalEntries * 32;
    return ((sizeInBytes + 1023) / 1024) * 1024;// نحوله لكلاسترز
...
bool Directory::canAddEntry(Directory_Entry d) {
    ...
    for (int i = 0; i < 1024; i++) {// نحسب الكلاسترز الفاضية
        if (Mini_FAT::FAT[i] == 0) {
            availableClusters++;
        }
    }
...
void Directory::writeDirectory() {
    ...
    if (total_clusters_needed > current_clusters.size()) {
        ...
        for (size_t i = 0; i < total_clusters_needed - current_clusters.size(); ++i) {
            int new_cluster = Mini_FAT::getAvailableCluster();
            if (new_cluster == -1) {
    ...
    for (auto cluster : current_clusters) {
        ...
        virtualDisk::writeCluster(cluster_data, cluster); // Write data to cluster
        data_offset += to_copy;
...
void Directory::readDirectory() {
    ...
    while (cluster_index != -1 && cluster_index != 0) {
        vector<char> cluster_data = virtualDisk::readCluster(cluster_index);
    ...
    for (size_t i = 0; i + entry_size <= directory_data.size(); i += entry_size) {
        ...
        if (entry.dir_name[0] != '\0') { // Check for valid entry
...
int Directory::searchDirectory(string name) {
    ...
    for (size_t i = 0; i < DirOrFiles.size(); ++i) {
        if (strcmp(DirOrFiles[i].dir_name, cleanedName.c_str()) == 0) {
            ...
            return i; // Return the index of the directory/file
        }
    }
    return -1; // Not found
}
